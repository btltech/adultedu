#!/usr/bin/env node
/**
 * Grade the question bank from learner behaviour instead of from reading it.
 *
 * Every attempt records which option the learner chose, so the bank can audit
 * itself: a question where most learners pick the same wrong option is far
 * more likely miskeyed than hard, and that shows up in the data long before
 * anyone clicks "report a problem". Reports capture the motivated few;
 * attempts capture everyone.
 *
 * Signals, strongest first:
 *   miskey_suspected   a single wrong option beats the keyed answer
 *   near_zero_success  almost nobody gets it right — usually broken, not hard
 *   guessing           answers spread evenly, so the item teaches nothing
 *   fast_wrong         wrong answers arrive too quickly to have been read
...
import { safeJsonParse } from './questionQualityUtils.js'
...
async function main() {
    ...
    const config = {
        minAttempts: num(args, 'min-attempts', DEFAULTS.minAttempts),
        nearZeroSuccess: num(args, 'near-zero-success', DEFAULTS.nearZeroSuccess),
        miskeyMargin: num(args, 'miskey-margin', DEFAULTS.miskeyMargin),
        guessingTolerance: num(args, 'guessing-tolerance', DEFAULTS.guessingTolerance),
        fastWrongSeconds: num(args, 'fast-wrong-seconds', DEFAULTS.fastWrongSeconds),
    ...
    const report = {
        ...
        mode: args.get('flag') === true ? 'flag' : 'audit',
        config,
    ...
    const outArg = [...args.keys()].find((key) => key === 'out')
    if (outArg) await writeFile(String(args.get('out')), JSON.stringify({ ...report, allFindings: findings }, null, 2))
