...
import { completeLabLevel, getLabProgress } from '../lib/api'
...
export default function LearningLab() {
    ...
    const LOCAL_PROGRESS_KEY = 'adultedu.labProgress.v1'

    const readLocalProgress = () => {
        if (typeof window === 'undefined') {
            return { graph: { completedIds: [] }, logic: { completedIds: [] } }
        }
        try {
            const raw = window.localStorage.getItem(LOCAL_PROGRESS_KEY)
            if (!raw) return { graph: { completedIds: [] }, logic: { completedIds: [] } }
            const parsed = JSON.parse(raw)
            const graphIds = Array.isArray(parsed?.graph?.completedIds) ? parsed.graph.completedIds : []
            const logicIds = Array.isArray(parsed?.logic?.completedIds) ? parsed.logic.completedIds : []

            const toCleanIds = (ids) =>
                [...new Set(ids.map((x) => Number(x)).filter((n) => Number.isInteger(n) && n > 0))].sort((a, b) => a - b)

            return {
                graph: { completedIds: toCleanIds(graphIds) },
                logic: { completedIds: toCleanIds(logicIds) },
            }
        } catch {
            return { graph: { completedIds: [] }, logic: { completedIds: [] } }
        }
    ...
    const graphCompletedSet = useMemo(() => new Set(progress.graph.completedIds), [progress.graph.completedIds])
    const logicCompletedSet = useMemo(() => new Set(progress.logic.completedIds), [progress.logic.completedIds])
    ...
    useEffect(() => {
        ...
        ; (async () => {
            try {
                const server = await getLabProgress()
                if (cancelled) return

                setProgress((prev) => {
                    const mergedGraph = [...new Set([...(prev.graph.completedIds || []), ...(server?.graph?.completedIds || [])])].sort(
                        (a, b) => a - b
                    )
                    const mergedLogic = [...new Set([...(prev.logic.completedIds || []), ...(server?.logic?.completedIds || [])])].sort(
                        (a, b) => a - b
                    )

                    const next = { graph: { completedIds: mergedGraph }, logic: { completedIds: mergedLogic } }
                    writeLocalProgress(next)
                    return next
                })
            } catch (e) {
                // ignore (user might not be logged in yet / cookie expired)
            }
        })()
    ...
    const markCompleted = async ({ labType, levelId }) => {
        const setForType = labType === 'graph' ? graphCompletedSet : logicCompletedSet
        if (setForType.has(levelId)) return

        setProgress((prev) => {
            const current = labType === 'graph' ? prev.graph.completedIds : prev.logic.completedIds
            const nextIds = [...new Set([...(current || []), levelId])].sort((a, b) => a - b)
            const next = labType === 'graph'
                ? { ...prev, graph: { completedIds: nextIds } }
                : { ...prev, logic: { completedIds: nextIds } }
            writeLocalProgress(next)
            return next
        })

        if (!isAuthenticated) {
            toast.success('Completed! Sign in to save your Lab progress.')
            return
        }

        try {
            const result = await completeLabLevel({ labType, levelId })
            const xpAwarded = result?.xp?.xpAwarded || 0
            toast.success(xpAwarded > 0 ? `Completed! +${xpAwarded} XP` : 'Completed!')
        } catch (e) {
            toast.error(e.message || 'Failed to save Lab progress')
        }
    }
        ...
        <div className="min-h-screen bg-gray-50 pb-20">
            ...
            <div className="container-app py-12">
                <div className="text-center mb-16">
                    ...
                    <h1 className="text-4xl font-bold text-dark-50 mb-4">Interactive Learning Lab 🧪</h1>
                    ...
                    <div className="max-w-3xl mx-auto bg-blue-50 border border-blue-100 rounded-xl p-6 text-left">
                        <h3 className="font-bold text-blue-800 mb-2 flex items-center gap-2">
                            <span className="text-xl">ℹ️</span> How to use the Lab
                        </h3>
                    ...
                    {!isAuthenticated && (
                        <div className="mt-6 max-w-3xl mx-auto bg-white border border-dark-100 rounded-xl p-5 text-left">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                <p className="text-sm text-dark-300">
                                    Want your Lab progress saved across devices? Sign in.
                                </p>
                ...
                <div className="space-y-24">
                    ...
                    <section className="bg-white p-8 rounded-3xl border border-dark-100 shadow-sm relative overflow-hidden">
                        ...
                        <div className="flex flex-col items-center gap-4 mb-8">
                            <div className="text-sm font-bold text-dark-300 uppercase tracking-widest">
                                Data Pool: {mathChallenges.length} Levels • Completed: {progress.graph.completedIds.length}/{mathChallenges.length}
                            </div>
                            ...
                            >
                                <span>🎲</span> Shuffle: Load Random Level
                            </button>
                        </div>
                        ...
                        <GraphPlotter
                            ...
                            onCorrect={() => markCompleted({ labType: 'graph', levelId: activeMathLevel.id })}
                        />
                    </section>
                    ...
                    <section className="bg-white p-8 rounded-3xl border border-dark-100 shadow-sm relative overflow-hidden">
                        ...
                        <div className="flex flex-col items-center gap-4 mb-8">
                            <div className="text-sm font-bold text-dark-300 uppercase tracking-widest">
                                Data Pool: {logicChallenges.length} Puzzles • Completed: {progress.logic.completedIds.length}/{logicChallenges.length}
                            </div>
                            ...
                            >
                                <span>🎲</span> Shuffle: Load Random Puzzle
                            </button>
                        </div>
                        ...
                        <LogicEngine
                            ...
                            onCorrect={() => markCompleted({ labType: 'logic', levelId: activeLogicLevel.id })}
                        />
                    </section>
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                        <div className="bg-white p-8 rounded-3xl border border-gray-200">
                            <h3 className="text-xl font-bold mb-4">🔬 Free Play: Statistics</h3>
                            <ProbabilityEngine />
                        </div>
                        <div className="bg-white p-8 rounded-3xl border border-gray-200">
                            <h3 className="text-xl font-bold mb-4">🍰 Free Play: Fractions</h3>
                            <FractionExplorer />
                        </div>
                    </div>
